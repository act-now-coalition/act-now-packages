...
export class Timeseries<T = unknown> {
  ...
  assertFiniteNumbers(): Timeseries<number> {
    this.points.forEach((p) => {
      assert(
        isFinite(p.value),
        `Found non-numeric (or non-finite) value in timeseries. date=${truncatedIsoString(
          p.date
        )} value=${p.value}`
      );
    });
  ...
  assertBoolean(): Timeseries<boolean> {
    this.points.forEach((p) => {
      assert(
        ...
        `Found non-boolean value in timeseries. date=${truncatedIsoString(
          p.date
        )} value=${p.value}`
      );
    });
  ...
  assertStrings(): Timeseries<string> {
    this.points.forEach((p) => {
      assert(
        ...
        `Found non-string value in timeseries. date=${truncatedIsoString(
          p.date
        )} value=${p.value}`
      );
    });
  ...
  toJSON(): TimeseriesPointJSON[] {
    const timeseriesPointJSONs: TimeseriesPointJSON[] = this.points.map(
      (point) => ({
        date: truncatedIsoString(point.date),
        value: point.value,
      })
    );
